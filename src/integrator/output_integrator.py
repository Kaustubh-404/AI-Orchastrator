...
class OutputIntegrator:
    ...
    async def integrate_multimedia_results(self, execution_result: Dict[str, Any], original_request: str) -> Dict[str, Any]:
        ...
        # Print task results for debugging
        print(f"Integrating multimedia results with {len(task_results)} tasks")
        for task_id, result in task_results.items():
            print(f"Task {task_id}: Status {result.get('status')}, Type: {type(result.get('result'))}")
        ...
        # Add any text content from the reasoning or text generation tasks
        if not text_content:
            for task_id, result in task_results.items():
                if result.get("status") == "completed":
                    task_data = result.get("result", {})
                    # Look for text in text generation tasks
                    if isinstance(task_data, dict) and "task_id" in task_data and "text" in task_data:
                        text_content += task_data["text"] + "\n\n"
        ...
        # Print the final response structure (without the actual data content)
        response_debug = {k: (v if not isinstance(v, str) or len(v) < 100 else f"[{len(v)} chars]") 
                        for k, v in response.items() if k != "raw_results"}
        print(f"Final multimedia response structure: {response_debug}")
